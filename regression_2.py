...
df = pd.read_csv(r'reference_0.92hz.csv')
df = df.dropna()
...
fields = ['latency', 'RSSI', 'filtered latency', "TIMESTAMP", "Predicted Drops"]
filename = "resultfile_" + latest_file
with open(filename, 'w') as csvfile:
    csvwriter = csv.writer(csvfile)
    csvwriter.writerow(fields)
    for a in range(0,len(new_Y)):
        row = [latency[a],RSSI[a],filt_latency[a],seconds[a],new_Y[a]]
